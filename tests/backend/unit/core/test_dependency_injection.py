...
for new routes, specifically testing the missing get_async_database_service dependency
...
class TestDependencyInjection:
    ...
    @pytest.mark.asyncio
    async def test_get_async_database_service_dependency_function_exists(self):
        """Test that get_async_database_service dependency function exists in core.dependencies."""
        try:
            from core.dependencies import get_async_database_service
            assert callable(get_async_database_service), "get_async_database_service should be a callable function"
        except ImportError:
            pytest.fail("get_async_database_service function should exist in core.dependencies module")

    @pytest.mark.asyncio
    async def test_get_async_database_service_returns_database_from_startup_service(self):
        """Test that get_async_database_service properly extracts database from startup service."""
        ...
        result = get_async_database_service(mock_startup_service)
        
        # Should return the database from the startup service
        assert result == mock_database, "get_async_database_service should return the database from startup service"

    @pytest.mark.asyncio
    async def test_get_async_database_service_raises_error_when_database_unavailable(self):
        """Test that get_async_database_service raises proper error when database is not available."""
        from core.dependencies import get_async_database_service
        ...
        with pytest.raises(HTTPException) as exc_info:
            get_async_database_service(mock_startup_service)
        
        assert exc_info.value.status_code == 503
        assert "Database service not available" in str(exc_info.value.detail)

    @pytest.mark.asyncio
    async def test_news_route_dependencies_can_be_resolved(self):
        ...
        try:
            ...
            assert callable(get_async_database_service), "get_async_database_service should be callable"
            
        except ImportError as e:
            pytest.fail(f"News route dependencies cannot be imported: {e}")

    @pytest.mark.asyncio
    async def test_data_items_route_dependencies_can_be_resolved(self):
    ...
    @pytest.mark.asyncio
    async def test_news_service_can_be_instantiated_with_database_dependency(self):
        ...
        try:
            ...
            mock_database = Mock(spec=AsyncDatabaseService)
    ...
    @pytest.mark.asyncio
    async def test_dependency_registry_has_required_providers(self):
    ...
    @pytest.mark.asyncio
    async def test_async_database_service_dependency_chain_works_end_to_end(self):
        """Test that the full dependency chain works from startup service to database service."""
        from core.dependencies import get_async_database_service, get_startup_service_dependency
        ...
        async_database_service = get_async_database_service(mock_startup_service)
        assert async_database_service == mock_database, "Dependency chain should work end-to-end"

    @pytest.mark.asyncio
    async def test_route_dependency_injection_patterns_are_consistent(self):
        ...
        try:
            from api.routes.calendar import get_async_database_service as calendar_get_db
            from api.routes.news import get_news_service
            from core.dependencies import get_async_database_service
            ...
            assert callable(get_async_database_service), "Core database dependency should be callable"
            
        except ImportError as e:
            pytest.fail(f"Dependency injection patterns are inconsistent: {e}")

    @pytest.mark.asyncio
    async def test_startup_service_dependency_is_available(self):
