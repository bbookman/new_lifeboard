"""
TDD Test for AsyncDatabaseService.get_available_dates implementation
...
class TestAsyncGetAvailableDates:
    """Test suite for get_available_dates async implementation"""
    
    @pytest.mark.asyncio
    async def test_get_available_dates_empty_database(self, async_temp_db_path):
        """RED: Test with empty database should return empty list"""
        db_service = AsyncDatabaseService(async_temp_db_path)
        await db_service.initialize()
        
        result = await db_service.get_available_dates()
        assert result == []
    
    @pytest.mark.asyncio
    async def test_get_available_dates_single_date(self, async_temp_db_path):
    ...
    @pytest.mark.asyncio
    async def test_get_available_dates_multiple_dates(self, async_temp_db_path):
        """RED: Test with multiple dates, should be sorted descending"""
        db_service = AsyncDatabaseService(async_temp_db_path)
        await db_service.initialize()
    ...
    @pytest.mark.asyncio
    async def test_get_available_dates_namespace_filter(self, async_temp_db_path):
        ...
        result = await db_service.get_available_dates(["news"])
        assert len(result) == 2
        assert "2025-01-15" in result
        assert "2025-01-14" in result
        assert result == ["2025-01-15", "2025-01-14"]  # Descending order
        
        # Filter by weather namespace only
        result_weather = await db_service.get_available_dates(["weather"])
        assert result_weather == ["2025-01-15"]
        
        # Filter by both namespaces
        result_both = await db_service.get_available_dates(["news", "weather"])
        assert len(result_both) == 2
        assert set(result_both) == {"2025-01-15", "2025-01-14"}
    
    @pytest.mark.asyncio
    async def test_get_available_dates_null_dates_filtered(self, async_temp_db_path):
    ...
    @pytest.mark.asyncio
    async def test_get_available_dates_distinct_dates(self, async_temp_db_path):
