...
class IngestionService(BaseService, ServiceDebugMixin):
    ...
    async def register_source(self, source: BaseSource):
        ...
        await self.database.register_data_source(
    ...
    async def process_pending_embeddings(self, batch_size: int = 32) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_items = await self.database.get_pending_embeddings(limit=batch_size * 2)
            ...
            for item, embedding in zip(items, embeddings):
                try:
                    ...
                    if success:
                        ...
                        await self.database.update_embedding_status(item['id'], 'completed')
                        result["successful"] += 1
                        logger.debug(f"Generated embedding for: {item['id']}")
                    else:
                        await self.database.update_embedding_status(item['id'], 'failed')
                        result["failed"] += 1
    ...
    async def _check_service_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_items = await self.database.get_pending_embeddings(limit=100)
            health_info["pending_embeddings"] = len(pending_items)
    ...
    async def get_ingestion_status(self) -> Dict[str, Any]:
        ...
        db_stats = await self.database.get_database_stats()
        pending_items = await self.database.get_pending_embeddings(limit=1000)
        ...
        source_stats = {}
        for namespace in self.sources.keys():
            items = await self.database.get_data_items_by_namespace(namespace, limit=1)
            last_sync = await self.database.get_setting(f"{namespace}_last_sync")
