...
class IngestionService(BaseService):
    ...
    async def ingest_from_source(self,
        ...
        try:
            ...
            if items:
                ...
                if hasattr(processor, "process_batch") and callable(processor.process_batch):
            ...
            error_msg = f"Ingestion failed for {namespace}: {e!s}"
    ...
    async def _store_processed_item(self, processed_item: DataItem, result: IngestionResult):
        ...
        try:
            ...
            error_msg = f"Error storing processed item {processed_item.source_id}: {e!s}"
            logger.error(error_msg)
            result.errors.append(error_msg)
    ...
    async def _process_and_store_item(self, item: DataItem, result: IngestionResult):
        ...
        try:
            ...
            error_msg = f"Error processing item {item.source_id}: {e!s}"
            logger.error(error_msg)
            result.errors.append(error_msg)
    ...
    async def process_pending_embeddings(self, batch_size: int = 32) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Error processing embeddings: {e!s}"
            logger.error(error_msg)
            result["errors"].append(error_msg)
        ...
        try:
            ...
            for item, embedding in zip(items, embeddings):
                try:
                    ...
                    self.database.update_embedding_status(item["id"], "failed")
                    result["failed"] += 1
                    result["errors"].append(f"Error processing {item['id']}: {e!s}")

                result["processed"] += 1

        except Exception as e:
            error_msg = f"Batch embedding failed: {e!s}"
            logger.error(error_msg)
            result["errors"].append(error_msg)
    ...
    async def ingest_items(self, namespace: str, data_items: List[DataItem]) -> IngestionResult:
        ...
        try:
            ...
            error_msg = f"Error during batch ingestion for {namespace}: {e!s}"
            logger.error(error_msg)
            result.errors.append(error_msg)
            result.end_time = datetime.now(timezone.utc)
