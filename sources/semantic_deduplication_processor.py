...
class SemanticDeduplicationProcessor(BaseProcessor):
    ...
    def _generate_theme(self, text: str) -> str:
        ...
        if any(word in text.lower() for word in ["meeting", "work", "project", "deadline"]):
            return "work_discussion"
        if any(word in text.lower() for word in ["tired", "exhausted", "energy", "sleep"]):
            return "energy_state"
        if any(word in text.lower() for word in ["food", "eat", "hungry", "lunch", "dinner"]):
            return "food_related"
