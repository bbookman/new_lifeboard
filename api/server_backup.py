    ...
    try:
        ...
        if result.get("success", False):
            ...
            if post_init_service:
                logger.info("LIFESPAN: Checking available services in startup service...")
...
def find_server_processes_alternative():
    ...
    try:
        ...
        grep_result = subprocess.run(["grep", "-E", r"python.*api/server\.py"],
                                   check=False, input=ps_result.stdout,
...
async def run_server(host: str = "0.0.0.0", port: int = 8000, debug: bool = False, kill_existing: bool = False):
    ...
    logger.info("=" * 60)
    logger.info("UVICORN: Starting Lifeboard API server")
    ...
    try:
        ...
        if server is not None:
            ...
            logger.info("UVICORN: Server instance stored for graceful shutdown")
            logger.info("UVICORN: Server starting with graceful shutdown timeout: 30s")
        else:
        ...
        if "address already in use" in str(e).lower():
            ...
            logger.error("UVICORN: Try killing existing processes: pkill -f 'python.*server.py'")
        raise
    finally:
...
def check_frontend_dependencies() -> bool:
    ...
    try:
        ...
        if not node_modules.exists():
            logger.warning("FRONTEND: node_modules not found, dependencies may need installation")
            return False
...
def start_frontend_server(port: int = 5173, backend_port: int = 8000) -> dict:
    ...
    try:
        ...
        if process.poll() is None:
            ...
            error_msg = "Frontend server failed to start"
            if stderr:
