  ...
  return (
    <div className="space-y-6">
      <div className="border-b-2 border-photo-accent pb-2">
        <h2 className="font-headline text-3xl font-bold text-newspaper-headline">Visual Diary</h2>
          ...
          <Card key={photo.id} className="overflow-hidden hover:shadow-lg transition-shadow group">
            <div className="aspect-[4/3] relative overflow-hidden">
              ...
              <div className="absolute top-3 right-3">
                <span className="bg-black/70 text-white px-2 py-1 rounded text-xs font-body">{photo.timestamp}</span>
              </div>
            </div>

            <div className="p-4">
              <h3 className="font-body font-semibold text-newspaper-headline mb-1">{photo.caption}</h3>
              {photo.location && <p className="text-newspaper-byline text-sm flex items-center">📍 {photo.location}</p>}
            </div>
          </Card>
        ))}
      </div>
