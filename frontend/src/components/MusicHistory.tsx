  ...
  return (
    <div className="space-y-6">
      <div className="border-b-2 border-music-accent pb-2">
        <h2 className="font-headline text-3xl font-bold text-newspaper-headline">Music Journal</h2>
      ...
      <Card className="p-6 bg-gradient-to-r from-music-accent/5 to-music-accent/10">
        <div className="flex items-center space-x-4 mb-6">
          <div className="w-20 h-20 rounded-lg overflow-hidden shadow-lg">
            <img src={musicImage} alt="Music collage" className="w-full h-full object-cover" />
          </div>
          <div>
            <h3 className="font-headline text-xl font-bold text-newspaper-headline">Today's Listening Stats</h3>
            <p className="text-newspaper-byline font-body">4 tracks • 19 minutes • Mostly chill vibes</p>
          </div>
        </div>
          ...
          {sampleTracks.map((track) => (
            ...
            >
              <div className="flex-1">
                <h4 className="font-body font-semibold text-newspaper-headline">{track.title}</h4>
              ...
              <div className="flex items-center space-x-3">
                {track.mood && <Badge className={`text-xs ${getMoodColor(track.mood)}`}>{track.mood}</Badge>}
                <span className="text-newspaper-byline text-sm font-mono">{track.duration}</span>
                <span className="text-newspaper-byline text-xs">{track.playTime}</span>
              </div>
            </div>
          ))}
        </div>
