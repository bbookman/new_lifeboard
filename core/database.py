...
from .json_utils import DatabaseRowParser, JSONMetadataParser
...
class DatabaseService:
    ...
    def get_markdown_by_date(self, date: str, namespaces: Optional[List[str]] = None) -> str:
            ...
            if item.get("metadata"):
                ...
                if isinstance(metadata, dict):
                    ...
                    if markdown_content:
                        ...
                        logger.info(f"[MARKDOWN DEBUG] Item {i+1}: Cleaned markdown preview: {markdown_content[:100]!r}")
                    ...
                    if not markdown_content:
                        ...
                        if not markdown_content:
                            ...
                            logger.info(f"[MARKDOWN DEBUG] Item {i+1}: Constructed markdown preview: {markdown_content[:100]!r}")
        ...
        if markdown_parts:
            ...
            logger.info(f"[MARKDOWN DEBUG] Final markdown preview: {combined_markdown[:200]!r}")
        ...
        logger.info(f"[MARKDOWN DEBUG] No content found, returning fallback: {fallback_content!r}")
